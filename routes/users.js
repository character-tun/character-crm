...
router.post('/', async (req, res) => {
  try {
    const { email, pass_hash, full_name, is_active } = req.body || {};
    if (DEV_MODE) {
      ...
      const created = { _id: nextId(), email, full_name: full_name || '', is_active: is_active !== false, createdAt: now, updatedAt: now, roles: [] };
      mem.users.unshift(created);
      return res.status(201).json(sanitizeUser(created));
    }
    const created = await User.create({ email, pass_hash, full_name, is_active });
    const lean = created.toObject();
    res.status(201).json(sanitizeUser(lean));
  } catch (err) {
...
router.put('/:id', async (req, res) => {
  try {
    const { email, full_name, is_active, roles } = req.body || {};
    if (DEV_MODE) {
