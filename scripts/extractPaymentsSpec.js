...
(function main() {
  ...
  if (!fs.existsSync(sourcePath)) {
    console.error(`[extractPaymentsSpec] Source not found: ${sourcePath}. Run scripts/generateSwagger.js first.`);
    process.exit(1);
  }
  ...
  const outSpec = {
    ...
    info: {
      title: `${(spec.info && spec.info.title) ? spec.info.title : 'API'} — Payments`,
      version: (spec.info && spec.info.version) ? spec.info.version : '1.0.0',
      description: 'Subset of OpenAPI spec for /api/payments endpoints.',
    },
  ...
  const outFile = path.join(outDir, 'payments.json');
  fs.writeFileSync(outFile, JSON.stringify(outSpec, null, 2), 'utf8');
  console.log(`[extractPaymentsSpec] Wrote ${outFile}`);
})();