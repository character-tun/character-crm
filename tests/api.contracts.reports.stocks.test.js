  ...
  aggregate: jest.fn(async (pipeline) => {
    ...
    if (groupId && typeof groupId === 'object' && groupId.itemId === '$itemId' && groupId.type === '$type') {
      ...
      for (const op of ops) {
        ...
        const t = String(op.type || '');
        const _k = `${id}:${t}`;
        const cur = map.get(_k) || 0;
        map.set(_k, cur + Number(op.qty || 0));
      }
      return Array.from(map.entries()).map(([_k, sum]) => {
        const [itemId, type] = _k.split(':');
...
const reportsRouter = require('../routes/reports/stocks');
...
function makeApp() {
  ...
  app.use(withUser);
  app.use('/api/reports/stocks', reportsRouter);
  return app;
}
...
beforeEach(() => { global.mockMem.balances = new Map(); global.mockMem.operations = []; });

describe('Контракты /api/reports/stocks/*', () => {
  test('GET /summary — агрегаты по локациям', async () => {
    seedBalance('i1', 'A', 10, 2);
    seedBalance('i2', 'B', 3, 1);
    seedBalance('i3', 'A', 5, 0);
    const app = makeApp();
    const res = await request(app).get('/api/reports/stocks/summary?limit=10');
    expect(res.statusCode).toBe(200);
    expect(res.body.ok).toBe(true);
    expect(Array.isArray(res.body.groups)).toBe(true);
    const a = res.body.groups.find((g) => g.locationId === 'A');
    const b = res.body.groups.find((g) => g.locationId === 'B');
    ...
    expect(res.body.totalQty).toBe(18);
  });

  test('GET /turnover — агрегаты по операциям и группировка byItem', async () => {
    ...
    const app = makeApp();
    const res = await request(app).get('/api/reports/stocks/turnover?limit=10');
    expect(res.statusCode).toBe(200);
    expect(res.body.ok).toBe(true);
    expect(res.body.totals.in).toBe(8);
    expect(res.body.totals.out).toBe(3);
    expect(res.body.totals.net).toBe(5);
    const g1 = res.body.byItem.find((g) => g.itemId === 'i1');
    const g2 = res.body.byItem.find((g) => g.itemId === 'i2');
    expect(g1.in).toBe(6);
  ...
  test('GET /turnover с from/to — фильтрация по периоду', async () => {
    ...
    const app = makeApp();
    const res = await request(app).get('/api/reports/stocks/turnover?from=2025-01-01&to=2025-01-31&limit=10');
    expect(res.statusCode).toBe(200);
    expect(res.body.ok).toBe(true);
    expect(res.body.totals.in).toBe(5); // only in Jan, 'return' in Feb excluded
    expect(res.body.totals.out).toBe(1);
    expect(res.body.totals.net).toBe(4);
    const g1 = res.body.byItem.find((g) => g.itemId === 'i1');
    expect(g1.in).toBe(5);
  ...
  test('Auth guard: 401 без пользователя', async () => {
    ...
    app.use(express.json());
    app.use('/api/reports/stocks', reportsRouter);
    const res = await request(app).get('/api/reports/stocks/summary?limit=5');
